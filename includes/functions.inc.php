...
function createUser($conn, $name, $email, $username, $bornDate, $pwd) {
  	$sql = "INSERT INTO users (uname, name, email, bornDate, type, pwd, profileImg, links, hobby, work, sport, music, visible) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
	...
	mysqli_stmt_bind_param($stmt, "ssssssssssssi", $username, $name, $email, $bornDate, $type, $hashedPwd, $profileImg, $links, $hobby, $work, $sport, $music, $visible);
	mysqli_stmt_execute($stmt);
	mysqli_stmt_close($stmt);
	mysqli_close($conn);
...
function updateUser($conn, $id, $name, $uname, $email, $bornDate, $type, $profileImg, $about, $links, $badge, $coupon, $level, $hobby, $work, $sport, $music, $security, $zip, $city, $addr, $phone, $visible) {
	...
	$sql = "UPDATE users SET uname=?, name=?, email=?,bornDate=?,type=?,profileImg=?,about=?,links=?,badge=?,coupon=?,level=?,hobby=?,work=?,sport=?,music=?, addr=?, phone=?, zip=?, city=? , visible=? WHERE id=?;";
	...
	mysqli_stmt_bind_param($stmt, "ssssssssssissssssisii", $uname, $name, $email, $bornDate, $type, $profileImg, $about, $links, $badge, $coupon, $level, $hobby, $work, $sport, $music, $addr, $phone, $zip, $city, $visible, $id);
	mysqli_stmt_execute($stmt);
	mysqli_stmt_close($stmt);
	mysqli_close($conn);
